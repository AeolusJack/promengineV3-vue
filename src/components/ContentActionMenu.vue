<template>
  <div class="relative inline-block">
    <button
      @click="toggleMenu"
      class="p-1 rounded hover:bg-hover-bg transition-colors"
    >
      <MoreHorizontal class="w-4 h-4 text-text-secondary" />
    </button>
    <div
      v-if="visible"
      class="absolute right-0 mt-1 w-40 bg-white rounded-card shadow-card border border-border-light z-50"
    >
      <button
        @click="handleAction('memory')"
        class="w-full px-4 py-2 text-left text-sm hover:bg-hover-bg flex items-center"
      >
...
const handleAction = (action: string) => {
  switch (action) {
    case 'memory':
      emit('saveToMemory', props.content)
      break
    case 'image':
      emit('exportImage', props.content)
      break
    case 'pdf':
      emit('exportPdf', props.content)
      break
    case 'share':
