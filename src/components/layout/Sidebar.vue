<template>
  <aside class="w-60 bg-bg-secondary flex flex-col h-full">
    <div class="h-16 flex items-center px-4">
      <span class="font-medium text-lg">{{ $t('brand') }}</span>
    </div>
    ...
    <div class="mt-auto px-3 pb-4 relative">
      <div
        ...
        @click="toggleMenu"
      >
        <img :src="userAvatar" alt="Avatar" class="w-8 h-8 rounded-full object-cover border border-border-light" />
      ...
      <Transition name="fade">
        ...
        >
          <button @click="logout" class="w-full flex items-center space-x-3 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors">
...
import { MessageSquare, Brain, Bot, Package, Wrench, Settings, ChevronUp, ChevronDown, LogOut } from 'lucide-vue-next'
...
const userAvatar = computed(() => `https://api.dicebear.com/7.x/avataaars/svg?seed=${username.value || 'user'}`)
...
const toggleMenu = () => { showLogoutMenu.value = !showLogoutMenu.value }
const closeMenu = (event: MouseEvent) => {
  if (!(event.target as HTMLElement).closest('.relative')) showLogoutMenu.value = false
}
const logout = () => {
  localStorage.removeItem('token'); localStorage.removeItem('userId'); localStorage.removeItem('username'); localStorage.removeItem('nickname')
  showLogoutMenu.value = false; router.push('/login')
}
onMounted(() => {
